    ...
    public class WavImporter : ContentImporter<AudioContent>
    {
        ...
        public override AudioContent Import(string filename, ContentImporterContext context)
        {
            ...
            if (content.Format.SampleRate < 8000 || content.Format.SampleRate > 48000)
                throw new InvalidContentException(string.Format(CultureInfo.InvariantCulture, "Audio file {0} contains audio data with unsupported sample rate of {1}Hz. Supported sample rates are from 8000Hz up to 48000Hz.", Path.GetFileName(filename), content.Format.SampleRate));
