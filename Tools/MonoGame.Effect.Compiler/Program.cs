    ...
    public static class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            var sourceDirectory = Path.GetDirectoryName(sourceFilepath)
                ?? throw new InvalidOperationException($"Could not determine the source directory for '{sourceFilepath}'.");
            ...
            try
            {
                ...
                if (writeHeader)
                {
                    ...
                    using (var binary = new MemoryStream())
                    {
                        ...
                        {
                            if (i % 12 == 0)
                                code.Append(Environment.NewLine).Append('\t');
        ...
        private class ConsoleEffectCompilerOutput : IEffectCompilerOutput
        {
            ...
            {
                throw new Exception(string.Format(CultureInfo.InvariantCulture, "{0}({1},{2}): error PREPROCESS01: {3}", ConvertToNative(file), line, column, ConvertMessage(message)));
            }
        }
    }
}
