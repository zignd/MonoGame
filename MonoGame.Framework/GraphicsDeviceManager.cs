...
namespace Microsoft.Xna.Framework
{
    ...
    public partial class GraphicsDeviceManager : IGraphicsDeviceService, IDisposable, IGraphicsDeviceManager
    {
        ...
        private void PreparePresentationParameters(PresentationParameters presentationParameters)
        {
            ...
            if (_preferMultiSampling)
            {
                ...
                presentationParameters.MultiSampleCount = GraphicsDevice != null
                    ? GraphicsDevice.PlatformGetMaxMultiSampleCount(_preferredBackBufferFormat)
                    : 32;
            }
            else
            {
        ...
        /// macOS / DesktopGL). When <c>true</c>, the window stays sized in logical points while the
        /// back buffer and viewport are sized in physical pixels, so rendering is crisp instead of
        /// being upscaled by the OS; <see cref="PreferredBackBufferWidth"/>/<see cref="PreferredBackBufferHeight"/>
        /// are interpreted as points. When <c>false</c> (the default) behaviour is unchanged.
        /// Must be set before the graphics device is created (e.g. in the Game constructor); it has
        /// no effect on platforms without a points/pixels distinction.
        /// </summary>
